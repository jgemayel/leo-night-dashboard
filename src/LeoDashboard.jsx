    ...
    <div style={{fontFamily:"'DM Sans',-apple-system,'Segoe UI',sans-serif",background:"#0b0b13",color:"rgba(255,255,255,0.85)",minHeight:"100vh",maxWidth:640,margin:"0 auto"}}>
      ...
      <div style={{padding:"28px 20px 20px",background:"linear-gradient(180deg, rgba(99,102,241,0.07) 0%, transparent 100%)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div><h1 style={{fontSize:20,fontWeight:800,margin:0,letterSpacing:"-0.02em",color:"#e0e7ff"}}>Leo's Night Report</h1><p style={{fontSize:12,color:"rgba(255,255,255,0.3)",margin:"3px 0 0",fontWeight:500}}>Jan 10 - Mar 20, 2026 \u00b7 62 nights \u00b7 7 PM - 7 AM</p></div>
        </div>
      </div>
...
{tab==="overview"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
  ...
  <div onClick={()=>goTo("feeding")} style={{...cd,cursor:"pointer"}}>
    ...
    <BenchBox color="#38bdf8"><p style={{fontSize:12,color:"rgba(255,255,255,0.4)",lineHeight:1.65,margin:0}}><span style={{color:"rgba(255,255,255,0.6)",fontWeight:700,fontSize:11,textTransform:"uppercase",letterSpacing:"0.04em"}}>vs. typical: </span>At 1-3 months, 120-180ml per feed every 2-4 hours is normal. His nighttime total plus daytime feeds likely puts daily intake in the expected 600-900ml range. The BM/FM shift is very common.</p></BenchBox>
  </div>
  ...
  <div onClick={()=>goTo("digestion")} style={{...cd,cursor:"pointer"}}>
    ...
    <BenchBox color="#fbbf24"><p style={{fontSize:12,color:"rgba(255,255,255,0.4)",lineHeight:1.65,margin:0}}><span style={{color:"rgba(255,255,255,0.6)",fontWeight:700,fontSize:11,textTransform:"uppercase",letterSpacing:"0.04em"}}>vs. typical: </span>Gas/colic peaks at 2-6 weeks and resolves by 3-4 months. Leo's 53% drop is exactly expected. Poop frequency dropping around 6-8 weeks is normal. Mar's uptick with more formula is common since formula digests differently.</p></BenchBox>
  </div>
  ...
  <div style={cd}>
    ...
    <ChartBars data={cData(data,"ls",fmt)} colorFn={v=>v>=300?"#4ade80":v>=240?"#22d3ee":v>=180?"#818cf8":v>=120?"#fbbf24":"#f87171"} max={400} h={140} rangeLow={120} rangeHigh={240} rangeLabel="Typical 1-3mo range (2-4h)" rangeColor="#818cf8"/>
...
{tab==="sleep"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
  ...
  <div style={cd}><h3 style={ct}>Longest Sleep Stretch per Night</h3><ChartBars data={cData(data,"ls",fmt)} colorFn={v=>v>=300?"#4ade80":v>=240?"#22d3ee":v>=180?"#818cf8":v>=120?"#fbbf24":"#f87171"} max={400} h={140} rangeLow={120} rangeHigh={240} rangeLabel="Typical 1-3mo (2-4h)" rangeColor="#818cf8"/></div>
  <div style={cd}><h3 style={ct}>Wake-ups per Night</h3><ChartBars data={cData(data,"wu")} colorFn={v=>v<=3?"#4ade80":v<=5?"#fbbf24":"#f87171"} max={12} h={110} rangeLow={2} rangeHigh={4} rangeLabel="Typical 1-3mo (2-4 wakes)" rangeColor="#fb923c"/></div>
...
{tab==="feeding"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
  ...
  <div style={cd}><h3 style={ct}>Volume per Feed</h3>
    ...
    <div style={{marginTop:6,padding:"6px 10px",background:"rgba(56,189,248,0.05)",borderRadius:8,border:"1px dashed rgba(56,189,248,0.15)"}}><span style={{fontSize:10,color:"rgba(56,189,248,0.6)",fontWeight:600}}>Typical range: 120-180 ml per feed at 1-3 months</span></div>
  </div>
</div>)}
